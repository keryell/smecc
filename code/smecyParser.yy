...
/* From there, we use the various methods of Attribute objects (see
   ...
   types of clause */
stream_loop_clause                      : STREAM_LOOP { Attribute::currentAttribute->addStreamLoop(); }
                                        ;
...
*/
if_clause				: IF '(' int ')' { Attribute::currentAttribute->addIf(Attribute::currentIntExpr); }
...
optional_int_list                       : /*empty*/ { Attribute::args.clear(); }
                                        | ',' int_list;
...
// ! Normalement on utilise les "Start Conditions" pour faire ça. Cf. la doc de flex, section 10.
...
[n:m] -> (n,m)
*/
range					: '/' '[' { Attribute::argRange.clear(); Attribute::isExprMode=1; } range_begin
					;


range_begin				: int { Attribute::currentPair.first = Attribute::currentIntExpr; } range_mid
					| ']' {
