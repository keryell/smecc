...
namespace smecy {
  ...
  */
  ...
  void addSmecySet(SgStatement* target,
                   ...
                   int instance_number) {
    {
      ...
      std::vector<SgExpression*> ev { copy(functionToMap),
                                      SageBuilder::buildIntVal(instance_number),
                                      copy(mapName) };
  ...
  */
  ...
  void translateMapping(SgProject *p) {
    // Generate a unique instance number so that we can differentiate
    // to different calls to the same function
    int instance_number = 0;
    // Keep track of the functions generated by the outliner
    std::vector<std::pair<std::string,int>> generated_functions;
    // For all files of the project:
    for (auto * f : p->get_fileList()) {
      // Preprocessing
      correctParentBody(f);
      ...
      for(auto pragma : SmecyPragma(f)) {
        ...
        if (attribute->hasMapClause()) {
          ...
          translateMap(target, attribute, subject, ++instance_number);
          if (isAccelerator) {
            // Get the reference expression of the called function
            // after the #pragma
            SgExpression* f = getFunctionRef(subject);
            std::string function_name = f->unparseToString();
            // Keep track of the generated function name and instance
            generated_functions.push_back(std::pair<std::string,int>{
                function_name, instance_number});
...
//    auto func_decls = NodeQuery::querySubTree(p, V_SgFunctionDeclaration);
//    for(auto &node : func_decls) {
//      auto const decl = isSgFunctionDeclaration(node);
//      std::cout << decl->get_qualified_name().str ()
//                    << " --> " << decl->get_mangled_name().str ()
//                    << (decl->get_qualified_name().str.c () == std::string("::main"))
//                    << std::endl;
//      if (decl->get_qualified_name().str () == std::string("::main")) {
...
//    }
	...
	void translateMap(SgStatement* target, Attribute* attribute,
	                  SgStatement* functionToMap, int instance_number)
	{
		...
		addSmecySet(target, mapName, mapCoordinates,
		            getFunctionRef(functionToMap), instance_number);
