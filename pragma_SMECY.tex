...
\begin{description}
\item[\texttt{\#pragma smecy stream\_loop}:] this indicated the loop
  that must be turned into a stream of processes;

\item[\texttt{\#pragma smecy stream\_node(n)}:] this acts as a separator
  between groups of statements. It decides the node boundaries. Only data
  passing over these separators is turned into communication.
\end{description}
