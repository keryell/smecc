...
% explain the OpenMP bits in the input codes
%FIXME FIXME FIXME
...
#pragma omp parallel for schedule(static, 1)
    ...
    \marginpar{RK: Passer aussi cette section dans la section 1}
...
\section{Building a SME-C compiler}
	...
	%TODO modify the intro to make it make sense
...
int main() {
	...
	\begin{itemize}
		\item If an \verb+If+ clause is present then the API calls and
          ...
          \marginpar{RK: je pense que c'est incompréhensible si on ne
            connaît pas ce que c'est que la clause If. Rajouter plus de
            description des pragmas avec un exemple de "if" dans la
            section 1 SME-C? Du coup pour moi la section 3.2.1 devrait
            aussi être dans l'introduction de la problematique, section 1}
...
if (condition)
	...
	a = f();
		...
		\item If the pragma itself is the single-statement body of a
          structure \marginpar{RK: je ne comprends pas "body of a
          ...
          declaration.\marginpar{RK: je ne comprends pas "body of the pragma
          declaration". Donner un exemple? C'est quoi cette structure?
          \texttt{struc} en C?}
...
if (condition)
	...
	%TODO move a part to the SMECY features
